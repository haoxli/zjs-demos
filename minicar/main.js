...
var UltrasonicSensorDistance = 10; //cm
...
var UltrasonicSensorTimer = setInterval(function () {
    ...
    for (var i = 0; i < 100; i++) {
        if (UltrasonicSensorIn.read())  UltrasonicSensorCount++;
    }
...
var AccSensor = new Accelerometer({ frequency: 50 });

var AccValueY = 0, AccValueYOld = 0, AccValueYNew = 0, AccValue = 0;

AccSensor.onactivate = function() { console.log("Acc sensor: is activated"); };
AccSensor.start();
...
var TrackTimer = setInterval(function () {
    ...
    if (!UltrasonicSensorFlag && !bleFlag && trackFlag) {
        if (MiddleValue == 0 && (Right1Value == 0 || Left1Value == 0) && Right2Value == 1 && Left2Value == 1) {
            trackLostFlag = false;
            trackLostCount = 0;
        ...
        } else if ((MiddleValue + Right1Value) == 0 && Left1Value == 1) {
            trackLostFlag = false;
            trackLostCount = 0;
             ...
             if (currentAngle !== turnAngle1) {
                 ...
                 console.log("Minicar Tracking - turn right " + currentAngle + "c");
             }
        } else if ((MiddleValue + Left1Value) == 0 && Right1Value == 1) {
            trackLostFlag = false;
            trackLostCount = 0;
            ...
            if (currentAngle !== turnAngle1) {
                ...
                console.log("Minicar Tracking - turn left " + currentAngle + "c");
            }
         } else if ((Right1Value + Right2Value) == 0 && MiddleValue == 1 && Left1Value == 1) {
            trackLostFlag = false;
            trackLostCount = 0;
         ...
         } else if ((Left1Value + Left2Value) == 0 && MiddleValue == 1 && Right1Value == 1) {
            trackLostFlag = false;
            trackLostCount = 0;
    ...
    if (!UltrasonicSensorFlag && !bleFlag && trackFlag && !trackLostFlag) {
        if (driver.getDriverState() === "park") {
            ...
            if (trackSpeed < thresholdSpeed) {
                ...
                trackSpeed = trackSpeed + 10;
                if (trackSpeed >= thresholdSpeed)  trackSpeed = thresholdSpeed;
            } else if (trackSpeed >= thresholdSpeed && trackSpeed <= peakSpeed) {
                if (AccValue > 5) {
                    trackSpeed = basicSpeed;
                    return;
                }
                trackSpeed = trackSpeed + 5;
            } else {
                trackSpeed = basicSpeed;
            }
    ...
    if (!UltrasonicSensorFlag && !bleFlag && trackFlag && !trackLostFlag) {
        if (Right2Value && Right1Value && MiddleValue &&
            Left1Value && Left2Value) {
            if (trackLostCount === loseTime + 2) trackLostCount = 0;
            trackLostCount++;
        }
...
var DriverCharacteristic = new ble.Characteristic({
    ...
    descriptors: [
        new ble.Descriptor({ uuid: "2901", value: "Driver" })
    ]
});
...
var SensorCharacteristic = new ble.Characteristic({
    ...
    properties: ["read"],
    descriptors: [
        new ble.Descriptor({ uuid: "2901", value: "Sensor"})
    ]
});
...
DriverCharacteristic.onWriteRequest = function(data, offset, withoutResponse, callback) {
    ...
    if (data.readUInt8(3) === 0x63) {
        ...
        if (!UltrasonicSensorFlag)  driver.forward(bleSpeed);
    } else if (data.readUInt8(3) === 0x72) {
        driver.reverse(bleSpeed);
    }

    console.log("Minicar BLE - receive basic data '" + data.toString('hex') + "'");

    callback(this.RESULT_SUCCESS);
};

SensorCharacteristic.onReadRequest = function(offset, callback) {
    var UltrasonicSensorBuffData = 0x66;
    if (UltrasonicSensorFlag)  UltrasonicSensorBuffData = 0x74;
    sensorBuffer.writeUInt8(UltrasonicSensorBuffData, 0);
    console.log("Minicar BLE - send sensor data '" + sensorBuffer.toString('hex') + "'");
